...
while running:
    ...
    if (player1_received and player2_received):
        ...
        if user_input != 0:
            ...
            cpu_text = large_font.render('They picked ' + convert(player1_move) + ' ... ' + rps_result(user_input,player1_move), True, (140, 150, 100), (255,255,255))
            cpu_textRect = cpu_text.get_rect()
            cpu_textRect.center = (SCREEN_WIDTH/2, (4*SCREEN_HEIGHT)/5)

        print('\n\n')
        print("You picked ", convert(user_input), ' and the they picked ', convert(player1_move))
        print('Result: ', rps_result(user_input,player1_move))
